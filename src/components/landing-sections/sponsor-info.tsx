import React, { Component } from 'react'
import { render } from 'react-dom';
import './sponsor.scss';

export default function SponsorInfo(prop:any){
        return(
            <div className="row mt-3">
                <div className="col-lg-7 col-sm-7 portfolio-item align-self-center">
                    <div className="row">
                    <div className=" col-lg-5 align-items-center align-self-center text-center">
                 
                        <a href={prop.sponsorWebsite} >
                   
                        <img className="card-img-top img-fluid sponsor-img" src={prop.sponsorLogo} /></a>
                        </div>
                        <div className=" col-lg-5 align-items-center align-self-center text-center">
                        <img className="card-img-top img-fluid sponsor-qr" src={prop.QRCode}  />
                    </div>
                    </div>
                </div>
                <div className="col-lg-5 col-sm-5 portfolio-item mt-2 align-self-center">
                    <div className="text-center align-items-center">
                        {prop.companyDescription}
                    </div>
                </div>
            </div>
    
    
        )
    }

